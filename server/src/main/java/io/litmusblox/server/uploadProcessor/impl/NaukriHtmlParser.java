...
public class NaukriHtmlParser implements HtmlParser {
    ...
    private void populateCandidateCompanyDetails(Candidate candidate, Document doc) {
        if(null != doc.getElementsByAttributeValueContaining("src", "star.png").first() && null != doc.getElementsByAttributeValueContaining("src", "star.png").first().parent().parent().nextElementSibling().nextElementSibling().text()) {
