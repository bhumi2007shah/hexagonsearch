...
public interface JobRepository extends JpaRepository<Job, Long> {
    ...
    @Query(value = "select * from job where status in ('Draft', 'Live') and id in (select job_id from job_candidate_mapping where id in (select jcm_id from hiring_manager_workspace where share_profile_id in (select id from jcm_profile_sharing_details where receiver_id =:hiringManagerId)))", nativeQuery = true)
